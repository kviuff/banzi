...
public class CRC16 {
    ...
    /**
     * crc16 计算
     * @param bytes 传入需要计算的数组
     * @return
     */
    public static String crc16(byte[] bytes){
        int crc = CRC16.calcCrc16(bytes);
        String c = String.format("0x%04x", crc);
        c = c.toUpperCase();
        return c.substring(c.indexOf("X")+1);
    }
    // 测试
    public static void main(String[] args) {
        // 0x02 05 00 03 FF 00 , crc16=7C 09
        byte[] a ={0x5C,(byte)0xCF,0x7F,(byte)0xB2,0x37,0x0A,0x03,0x00,0x01,0x00};
        int crc = CRC16.calcCrc16(a);
        String c = String.format("0x%04x", crc);
        c = c.toUpperCase();
        System.out.println(c.substring(c.indexOf("X")+1));
    }
}
